...
for (let i = 0; i < artists.length; i++){
  artists[i].name === "Vincent Van Dough" ? artists[i].name = "Vincent Van Gogh" : null;
}
for (let i = 0; i < artists.length; i++){
  artists[i].name === "Vincent Van Gogh" ? console.log(artists[i].name) : null;
}
...
function get20s(array){
  ...
  for (let i = 0; i < array.length; i++){
    let y1 = array[i].years.split(" - ")[0];
    let y2 = array[i].years.split(" - ")[1];
    y1 > 1900 && y1 < 2000 && y2 > 1900 && y2 < 2000 ? theReturn.push(array[i].name) : null
  }
...
function addArtist(array){
    let outArtist = {
      id: 20,
      name: "Anas Adel Abdelsalam", 
      years: `1996 - ${Date}`,
      genre: "Web Design", 
      nationality: "Middle-Eastern",
      bio: "You do not want to know lol",
    };
    array.push(outArtist);
    return array;
  }
...
function lotsOfArt(array){
  let theReturn = [];
  for (let i = 0; i < array.length; i++){
    Number(array[i].paintings) > 100 ? theReturn.push(array[i].name) : null
  }
